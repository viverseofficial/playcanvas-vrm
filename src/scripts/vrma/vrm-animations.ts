...
interface IAnimationSetting {
  ...
  interface IVRMAResource { stateName: string; animTrack: pc.AnimTrack; setting?: IAnimationSetting }
  ...
  export function createVRMAResources(
    ...
    vrmaAssets.forEach(vrmaAsset => {
        ...
        if(vrmAnimations) {
            const animTrack: pc.AnimTrack = createVRMAnimTrack(pcRef, vrmaAsset.stateName, vrmAnimations[0], humanoid, version);
