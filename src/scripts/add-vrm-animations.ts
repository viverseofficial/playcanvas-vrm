...
export const assignAnimation = (
  entity: pc.Entity,
  { name, resource, setting }: { name: string; resource: pc.Asset; setting: IAnimationSetting },
) => {
  if (entity.anim) {
...
//   class AddVrmAnimations extends scriptType {
//     // asset!: pc.Asset;
//     humanoid?: VRMHumanoid;
...
//     createAnimTrack(animTrack: pc.AnimTrack, animTrackRef) {
//       const inputs = animTrack.inputs.map((input) => new pc.AnimData(input.components, input.data));
//       const outputs = animTrack.outputs.map(
//         (output) => new pc.AnimData(output.components, output.data),
//       );
...
//       return new animTrackRef(animTrack.name, animTrack.duration, inputs, outputs, curves);
//     }

//     loadAnimation(animationAssets: IAnimationAsset[], animTrackRef) {
...
//       return animationAssets.map((animationAsset) => {
//         const animTrack = this.createAnimTrack(animationAsset.asset.resource, animTrackRef);
